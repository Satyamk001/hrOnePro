...
export interface SplashButtonProps {
  ...
  disabled?: boolean;
  title?: string;
  /** Width of the button (default: 120px — scaled for header use) */
  width?: number;
  /** Height of the button (default: 42px — scaled for header use) */
  height?: number;
...
/**
 * SplashButton — Adapted from meet-ui's splash-button component.
 * Features a splash/firework SVG animation on hover with depth shadow and character stagger.
 * Scaled down from the original (220×80) to fit the app header (120×42 default).
 * No lucide-react, no "use client", uses local cn utility.
 */
export function SplashButton({
  text = "Sync",
  bgColor = "var(--color-primary)",
  accentColor = "var(--color-primary-deep)",
    ...
    <>
      <style>{`
        .sb-${uid} {
          ...
          --accent: ${accentColor};
          --text: ${textColor};
          --shadow: color-mix(in srgb, ${bgColor}, transparent 40%);
          --radius: 12px;
        ...
        .sb-${uid} .sb-bg::before {
          ...
          box-shadow:
            -4px 3px 0 0 var(--shadow),
            -8px 6px 0 0 color-mix(in srgb, var(--shadow), transparent 20%),
            -12px 9px 2px 0 color-mix(in srgb, var(--shadow), transparent 40%),
            -16px 12px 4px 0 color-mix(in srgb, var(--shadow), transparent 60%),
            -20px 15px 6px 0 color-mix(in srgb, var(--shadow), transparent 75%);
        }
        .sb-${uid} .sb-wrap {
          border-radius: inherit;
          overflow: hidden;
        ...
        .sb-${uid} .sb-content {
          ...
          background: linear-gradient(to bottom, var(--accent) 0%, var(--bg) 100%);
          box-shadow:
            inset -1px 6px 6px -3px color-mix(in srgb, var(--accent), transparent 30%),
            inset 1px -2px 6px 0px rgba(0,0,0,0.35);
        }
        ...
        .sb-${uid} .sb-icon {
          animation: sb-resetArrow-${uid} 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards;
          z-index: 10;
        }
        .sb-${uid} .sb-arrow,
        ...
        .sb-${uid} .sb-arrow::before {
          transform: rotate(44deg); top: 0px;
          box-shadow: 1px -1px 2px -1px var(--bg);
        ...
        .sb-${uid} .sb-arrow::after {
          ...
          box-shadow: -1px 1px 2px 0 var(--bg);
        ...
        .sb-${uid}:active .sb-bg::before {
          ...
          box-shadow:
            -4px 3px 0 0 var(--shadow),
            -8px 6px 0 0 color-mix(in srgb, var(--shadow), transparent 40%),
            -12px 9px 2px 0 color-mix(in srgb, var(--shadow), transparent 70%);
        }
        .sb-${uid}:active .sb-content {
          box-shadow:
            inset -1px 6px 4px -3px rgba(0,0,0,0.3),
            inset 0px -2px 4px 0px var(--accent);
        }
        .sb-${uid}:active .sb-outline { opacity: 0; }
        .sb-${uid}:active .sb-wrap { transform: translate(1.5px, -1.5px); }
        ...
        @keyframes sb-swingArrow-${uid} { 50% { transform: translateX(3px) scale(0.9); } }
        ...
        @keyframes sb-resetArrow-${uid} { 0% { transform: translateX(-60px); } 100% { transform: translateX(0); } }
